...
// Configuration
$conn = new mysqli(global_mysql_server, global_mysql_user, global_mysql_password, global_mysql_database); 
...
function get_configuration($data, $conn)
{
	$query = mysqli_query($conn,"SELECT * FROM " . global_mysql_configuration_table)or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysqli_error($conn)) . '</span>');
...
function user_name_exists($user_name, $conn)
{
...
function user_email_exists($user_email, $conn)
{
	$sql="SELECT * FROM " . global_mysql_users_table . " WHERE user_email='$user_email'";
 $query = mysqli_query($conn, $sql);
	if(mysqli_num_rows($query) > 0)
	{
		return(true);
...
function login($user_email, $user_password, $user_remember, $conn)
{
	...
	$sql = "SELECT * FROM " . global_mysql_users_table . " WHERE user_email='$user_email' AND user_password='$user_password_encrypted' OR user_email='$user_email' AND user_password='$user_password'";
	$query = mysqli_query($conn, $sql);
...
function check_login($conn)
{
	if(isset($_SESSION['logged_in']))
	{
		...
		$sql = "SELECT * FROM " . global_mysql_users_table . " WHERE user_id='$user_id'";
		$query = mysqli_query($conn, $sql);
...
function create_user($user_name, $user_email, $user_password, $user_secret_code, $conn)
{
	...
	elseif(user_name_exists($user_name, $conn) == true)
	{
	...
	{ 	$sql="SELECT * FROM " . global_mysql_users_table . "";
		...
		$sql="INSERT INTO " . global_mysql_users_table . " (user_is_admin,user_email,user_password,user_name,user_reservation_reminder) VALUES ($user_is_admin,'$user_email','$user_password','$user_name','0')";
...
{
	$query = mysqli_query($conn, "SELECT * FROM " . global_mysql_users_table . " WHERE user_is_admin='1' ORDER BY user_name")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
...
{   $conn = mysqli_connect(global_mysql_server, global_mysql_user, global_mysql_password, global_mysql_database);
	$query = mysqli_query($conn, "SELECT * FROM " . global_mysql_reservations_table . " WHERE reservation_week='$week' AND reservation_day='$day' AND reservation_time='$time'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
	$reservation = mysqli_fetch_array($query, MYSQLI_ASSOC);
...
function make_reservation($week, $day, $time)
{
	...
	{
		mysqli_query($conn, "INSERT INTO " . global_mysql_reservations_table . " (reservation_made_time,reservation_week,reservation_day,reservation_time,reservation_price,reservation_user_id,reservation_user_email,reservation_user_name) VALUES (now(),'$week','$day','$time','$price','$user_id','$user_email','$user_name')")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');

		return(1);
		
	}
	elseif($week < global_week_number && $_SESSION['user_is_admin'] != '1')
	{
		return($week.'You can\'t reserve back in time');
	}
	...
	else
	{
		...
		if(mysqli_num_rows($query) < 1)
		{
			...
			mysqli_query($conn, "INSERT INTO " . global_mysql_reservations_table . " (reservation_made_time,reservation_year,reservation_week,reservation_day,reservation_time,reservation_price,reservation_user_id,reservation_user_email,reservation_user_name) VALUES (now(),'$year','$week','$day','$time','$price','$user_id','$user_email','$user_name')")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
...
{
	$conn = mysqli_connect(global_mysql_server, global_mysql_user, global_mysql_password, global_mysql_database);
	if($week < global_week_number && $_SESSION['user_is_admin'] != '1' )
	{
		return('Ne možete poništiti protekle rezervacije');
	}
	...
	else
	{
		...
		{
			mysqli_query($conn, "DELETE FROM " . global_mysql_reservations_table . " WHERE reservation_week='$week' AND reservation_day='$day' AND reservation_time='$time'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');

			return(1);
		}
		else
		{
			return('Ne možete ukloniti rezervacije drugih korisnika');
		}
	}
}
...
{
	$query = mysqli_query($conn, "SELECT * FROM " . global_mysql_users_table . " ORDER BY user_is_admin DESC, user_name")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
...
function reset_user_password($user_id)
{
	...
	mysqli_query($conn, "UPDATE " . global_mysql_users_table . " SET user_password='$password_encrypted' WHERE user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
...
function change_user_permissions($user_id)
{
	...
	{
		mysqli_query($conn, "UPDATE " . global_mysql_users_table . " SET user_is_admin = 1 - user_is_admin WHERE user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
...
function delete_user_data($user_id, $data)
{
	...
	else
	{
		...
		{
			mysqli_query($conn, "DELETE FROM " . global_mysql_reservations_table . " WHERE reservation_user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
		}
		elseif($data == 'user')
		{
			mysqli_query($conn, "DELETE FROM " . global_mysql_users_table . " WHERE user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
			mysqli_query($conn, "DELETE FROM " . global_mysql_reservations_table . " WHERE reservation_user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
		}
...
function delete_all($data)
{
	...
	{
		mysqli_query($conn, "DELETE FROM " . global_mysql_users_table . "")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
		mysqli_query($conn, "DELETE FROM " . global_mysql_reservations_table . "")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
	}
...
function save_system_configuration($price)
{
	...
	{
		mysqli_query($conn, "UPDATE " . global_mysql_configuration_table . " SET price='$price'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
	}
...
function get_usage($conn)
{
	$usage = '<table id="usage_table"><tr><th>Reservations</th><th>Cost</th><th>Current price per reservation</th></tr><tr><td>' . count_reservations($_SESSION['user_id'], $conn) . '</td><td>' . cost_reservations($_SESSION['user_id'],$conn ) . ' ' . global_currency . '</td><td>' . global_price . ' ' . global_currency . '</td></tr></table>';
	return($usage);
}

function count_reservations($user_id, $conn)
{
	$query = mysqli_query($conn, "SELECT * FROM " . global_mysql_reservations_table . " WHERE reservation_user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
	$count = mysqli_num_rows($query);
	return($count);
}

function cost_reservations($user_id, $conn)
{
	$query = mysqli_query($conn, "SELECT * FROM " . global_mysql_reservations_table . " WHERE reservation_user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
...
{
	$user_id = $_SESSION['user_id'];
	$query = mysqli_query($conn, "SELECT * FROM " . global_mysql_users_table . " WHERE user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
...
{
	$user_id = $_SESSION['user_id'];
	mysqli_query($conn, "UPDATE " . global_mysql_users_table . " SET user_reservation_reminder = 1 - user_reservation_reminder WHERE user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
...
function change_user_details($user_name, $user_email, $user_password)
{
	...
	else
	{
		...
		else
		{
			...
			mysqli_query($conn, "UPDATE " . global_mysql_users_table . " SET user_name='$user_name', user_email='$user_email', user_password='$user_password' WHERE user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars( mysqli_error($conn)) . '</span>');
		}
