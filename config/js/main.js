// Show pages
function brisanje()
{if (confirm("Želite li izbrisati ovaj kolegij?")== true)
 {
	$('form#brisanje').submit();
 } 
}
function showabout()
{
	...
	$.get('config/about.php', function(data) { $('#content_div').html(data); div_fadein('#content_div'); page_loaded('about'); });
}
...
function showlogin()
{
	...
	$.get('config/login.php', function(data)
	{
...
function shownew_user()
{
	page_load();
	div_hide('#content_div');
	$.get('config/login.php?new_user', function(data) { $('#content_div').html(data); div_fadein('#content_div'); page_loaded(); input_focus('#user_name_input'); });
...
function showforgot_password()
{
	page_load();
	div_hide('#content_div');
	$.get('config/login.php?forgot_password', function(data) { $('#content_div').html(data); div_fadein('#content_div'); page_loaded(); });
...
{   
	...
	$.get('config/reservation.php', function(data)
	{
		$('#content_div').html(data);
		div_fadein('#content_div');

		$.get('config/reservation.php?week='+global_week_number, function(data)
		{
...
{   
	if(week == 'next')
	{
		var t = Date.parse($('#week_number_span').html()) + 86400000;
	...
	else if(week == 'inputField')
	{
		var t = Date.parse(kal);
    ...
    var week = new Date(t).toISOString().slice(0,10);
...
function showcp()
{
	page_load();
	div_hide('#content_div');
	$.get('config/cp.php', function(data) { $('#content_div').html(data); div_fadein('#content_div'); page_loaded(); });
}
...
function showhelp()
{
	page_load();
	div_hide('#content_div');
	$.get('config/help.php', function(data) { $('#content_div').html(data); div_fadein('#content_div'); page_loaded(); });
}
...
{
	// All
	$.get('config/main.php?day_number', function(data)
	{
	...
	if(page == 'about')
	{
		$('#about_latest_version_p').html('<img src="config/img/loading.gif" alt="Loading"> Getting latest version...');

		setTimeout(function()
		{
			$.get('config/main.php?latest_version', function(data)
			{
...
function login()
{
	...
	$('#login_message_p').html('<img src="config/img/loading.gif" alt="Loading"> Logging in...').slideDown('fast');
	...
	$.post('config/login.php?login', { user_email: user_email, user_password: user_password, user_remember: user_remember }, function(data)
	{
		if(data == 1)
		{
			input_focus();
...
function logout()
{
	...
	$.get('config/login.php?logout', function(data) { setTimeout(function() { window.location.replace('.'); }, 1000); });
}
...
function create_user()
{
	...
	{
		$('#new_user_message_p').html('<img src="config/img/loading.gif" alt="Loading"> Creating user...').slideDown('fast');

		$.post('config/login.php?create_user', { user_name: user_name, user_email: user_email, user_password: user_password, user_secret_code: user_secret_code }, function(data)
		{
			if(data == 1)
			{
				input_focus();
				...
				setTimeout(function()
				{
					$('#new_user_message_p').html('User created successfully! Logging in... <img src="config/img/loading.gif" alt="Loading">');
...
function toggle_reservation_time(id, week, day, time, from)
{
	...
	if(user_name == '')
	{
		...
		$.post('config/reservation.php?make_reservation', { week: week, day: day, time: time }, function(data) 
		{
			if(data == 1)
			{
		...
		if(offclick_event == 'mouseup' || from == 'details')
		{
			...
			else if(user_name == session_user_name || session_user_is_admin == '1')
			{
				...
				if(delete_confirm)
				{
					...
					$.post('config/reservation.php?delete_reservation', { week: week, day: day, time: time }, function(data)
					{
						if(data == 1)
						{
							setTimeout(function() { read_reservation(id, week, day, time); }, 1000);
...
{
	$.post('config/reservation.php?read_reservation', { week: week, day: day, time: time }, function(data) { $(id).html(data); });
}
...
function read_reservation_details(id, week, day, time)
{
	if(typeof id != 'undefined' && $(id).html() != '' && $(id).html() != 'Wait...')
	{
		if($('#reservation_details_div').is(':hidden'))
		{
			...
			$.post('config/reservation.php?read_reservation_details', { week: week, day: day, time: time }, function(data)
			{
				setTimeout(function()
				{
					if(data == 0)
...
function list_users()
{
	$.get('config/cp.php?list_users', function(data) { $('#users_div').html(data); });
}
...
function reset_user_password()
{
	if(typeof $(".user_radio:checked").val() !='undefined')
	{
		...
		$('#user_administration_message_p').html('<img src="config/img/loading.gif" alt="Loading"> Resetting password...').slideDown('fast');

		$.post('config/cp.php?reset_user_password', { user_id: user_id }, function(data)
		{
			if(data == 0)
			{
...
function change_user_permissions()
{
	if(typeof $(".user_radio:checked").val() !='undefined')
	{
		...
		$('#user_administration_message_p').html('<img src="config/img/loading.gif" alt="Loading"> Changing permissions...').slideDown('fast');
...
function delete_user_data(delete_data)
{
	if(typeof $(".user_radio:checked").val() !='undefined')
	{
		...
		if(delete_confirm)
		{
			...
			$('#user_administration_message_p').html('<img src="config/img/loading.gif" alt="Loading"> Deleting...').slideDown('fast');

			$.post('config/cp.php?delete_user_data', { user_id: user_id, delete_data: delete_data }, function(data)
			{
				if(data == 1)
				{
					setTimeout(function()
...
function delete_all(delete_data)
{
	...
	if(delete_confirm)
	{
		$('#database_administration_message_p').html('<img src="config/img/loading.gif" alt="Loading"> Deleting...').slideDown('fast');

		$.post('config/cp.php?delete_all', { delete_data: delete_data }, function(data)
		{
			if(data == 1)
			{
				setTimeout(function()
...
function save_system_configuration()
{
	...
	$('#system_configuration_message_p').html('<img src="config/img/loading.gif" alt="Loading"> Saving...');
	$('#system_configuration_message_p').slideDown('fast');

	$.post('config/cp.php?save_system_configuration', { price: price }, function(data)
	{
		if(data == 1)
		{
			input_focus();
...
function get_usage()
{
	$.get('config/cp.php?get_usage', function(data) { $('#usage_div').html(data); });
}

function get_reservation_reminders()
{
	$.get('config/cp.php?get_reservation_reminders', function(data) { $('#reservation_reminders_span').html(data); });
}

function add_one_reservation()
{
	$('#usage_message_p').html('<img src="config/img/loading.gif" alt="Loading"> Saving...').slideDown('fast');

	$.post('config/reservation.php?make_reservation', { week: '0', day: '0', time: '0' }, function(data)
	{
		if(data == 1)
		{
			setTimeout(function()
...
{
	$('#settings_message_p').html('<img src="config/img/loading.gif" alt="Loading"> Saving...').slideDown('fast');

	$.post('config/cp.php?toggle_reservation_reminder', function(data)
	{
		if(data == 1)
		{
			setTimeout(function()
...
function change_user_details()
{
	...
	{	
		$('#user_details_message_p').html('<img src="config/img/loading.gif" alt="Loading"> Saving and refreshing...').slideDown('fast');

		$.post('config/cp.php?change_user_details', { user_name: user_name, user_email: user_email, user_password: user_password }, function(data)
		{
			if(data == 1)
			{
				input_focus();
...
function hash()
{
	...
	if(hash == '')
	{
		if(typeof session_logged_in != 'undefined')
		{
			page_load('');
	        div_hide('#content_div');
			$.get('view/naslovna.php', function(data) { $('#content_div').html(data); div_fadein('#content_div'); page_loaded('about'); });
...
$(window).load(function()
{
	...
	{
		window.location.replace('config/error.php?error_code=3');
	}
	else
	{
